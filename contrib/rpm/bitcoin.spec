...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1852
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 1853
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11852
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11853
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 1852
	%{_sbindir}/semanage port -d -p tcp 1853
	%{_sbindir}/semanage port -d -p tcp 11852
	%{_sbindir}/semanage port -d -p tcp 11853
	for selinuxvariant in %{selinux_variants}; do
