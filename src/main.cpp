...
#if defined(NDEBUG)
# error "Gatorcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Gatorcoin Signed Message:\n";
...
{
    // Gatorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 262800); // Gatorcoin: 262.8k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Gatorcoin: 0.5 days
static const int64 nTargetSpacing = 8 * 60; // Gatorcoin: 8 minutes, after the 8 libraries
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gatorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gatorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043c78ddd0a936c799f3245b65d27aff5a782a2c95181bfa09fccc691aab7682dc163af432c40f45458e909fa2e3a47508e1c0ce956cb48b274ca3eea0a05d0be9") << OP_CHECKSIG;
        CBlock block;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GatorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GatorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GatorcoinMiner(CWallet *pwallet)
{
    printf("GatorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gatorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GatorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GatorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GatorcoinMiner, pwallet));
}
